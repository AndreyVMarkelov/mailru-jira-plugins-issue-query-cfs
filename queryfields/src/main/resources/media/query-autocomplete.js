...
(function($) {
	$.fn.cfsqueryautocomplete = function(cfId, issueId, baseUrl, restUrlPart,
			minlength) {
		callback = typeof minlength == "function" ? minlength
				: (typeof callback == "function" ? callback : function() {
				});
		minlength = !isNaN(Number(minlength)) ? minlength
				: QUERY_AUTOCOMPLETE_STARTS_AFTER;
				...
				jQuery
						.ajax({
							...
							data : {
								...
								"rowcount" : QUERY_MAX_DISPLAY_ROWS + 1
							...
							success : function(data) {
								...
								if (data != null) {
									...
									var i = 0;
									while (i < data.length
											&& displayRowsCount < QUERY_MAX_DISPLAY_ROWS) {
				...
				jQuery
						.ajax({
							...
							success : function(data) {
								...
								if ($('.' + cfId
										+ queryRepresentationTagPostfix).length > 0) {
									if (data) {
											...
											if (representation.children('#'
												...
												multiSelect
														.html(multiSelect
																...
																+ "_"
																+ value
																+ "\" class=\"mail-issue-element\" value=\""
																+ value
																+ "\" selected=\"selected\">"
																+ value
																+ "</option>");
